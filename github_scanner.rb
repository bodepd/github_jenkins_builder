...
class GithubScanner
    ...
    def initialize(options = {})
      ...
      @pr_cache   = TorqueBox::Infinispan::Cache.new(
        ...
        #:persist => '/tmp/processed_pull_requests'
      )
        ...
        v.each do |pr_num|
          ...
          unless @pr_cache.get(id)
            ['ubuntu', 'redhat'].each do  |os|
              ['puppet_openstack', 'tempest_smoke'].each do |test_mode|
                unless os == 'redhat' and test_mode == 'tempest_smoke'
            ...
            puts "putting #{id} into pull request cache"
